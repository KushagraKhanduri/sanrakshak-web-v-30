...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "rounded-2xl border bg-card text-card-foreground transition-transform duration-300 hover:scale-[1.02] hover:rounded-2xl relative z-10",
        isDark ? "border-white/10 bg-black/40 backdrop-blur-sm shadow-sm" : "border-gray-200 shadow-soft",
        className
      )}
      {...props}
    />
